...
parseMap str
  ...
  | otherwise = normalizeBoard cars -- Normaliza el tablero ordenando los coches por su ID, para mantener un orden consistente
  where
...
moveVehicle board car =
  ...
  where
    ...
    isValidMove occSet posList =
      all inBounds posList && all (`Set.notMember` occSet) posList
    ...
    replaceCar b newCar = newCar : filter ((/= carId newCar) . carId) b
...
showBoard cars = unlines [[Map.findWithDefault 'o' (r, c) posMap | c <- [0 .. 5]] | r <- [0 .. 5]]
  where
    posMap = Map.fromList [(pos, carId car) | car <- cars, pos <- positions car]
